...
export default function GitHubCallback() {
  ...
  const { loginWithGitHub, setAuthSession } = useAuth();
  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      if (directToken) {
        setAuthSession(directToken, {
          ...
          email: 'github.user@campusconnect.edu'
        });
        setTimeout(() => {
      ...
      const authCode = code || 'demo_github_student';
      ...
      if (state) {
        try {
          ...
          if (state === 'recruiter' || state === 'student') targetRole = state;
        }
      }

      try {
        const res = await loginWithGitHub(authCode, state, targetRole);
    ...
    processCallback();
  }, [location.search, loginWithGitHub, setAuthSession, navigate]);
  ...
  return (
    <AuthLoadingScreen
      title="Completing GitHub Verification"
